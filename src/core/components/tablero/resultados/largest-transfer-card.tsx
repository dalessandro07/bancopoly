...
import { ArrowRightIcon, BanknoteIcon, ClockIcon } from "lucide-react";
...
export function LargestTransferCard({
	largestTransfer,
}: LargestTransferCardProps) {
	...
	const numBills = Math.min(
		Math.max(Math.floor(largestTransfer.amount / 500), 3),
		6,
	);
			...
			<CardContent className="relative z-10 space-y-6">
				...
				<div className="flex items-center gap-3 p-3 rounded-lg bg-blue-50/50 dark:bg-blue-950/20 border border-blue-200/50 dark:border-blue-800/50">
					...
					<div className="flex-1">
						...
						<p className="font-bold text-lg">
							{largestTransfer.fromPlayer.name}
						</p>
					</div>
				</div>
				...
				<div className="relative">
					...
					<div className="text-center space-y-2">
						...
						{largestTransfer.createdAt && (
							<div className="flex items-center justify-center gap-1.5 mt-2 text-xs text-muted-foreground">
								<ClockIcon className="size-3.5" />
								<span>{formatDate(largestTransfer.createdAt)}</span>
							</div>
						)}
					</div>
				</div>
