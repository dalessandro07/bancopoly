			...
			<CardContent className="space-y-6">
				...
				{topThree.length > 0 && (
					<div className="relative pt-6 overflow-visible">
						{/* Podio - pt-6 da espacio para el icono de copa del 1er puesto */}
						<ScrollArea className="w-full">
							<div className="flex items-end justify-center gap-2 md:gap-4 px-2 min-w-max pt-6">
								{/* 2do lugar - Izquierda */}
								{podiumOrder[0] && (
									<div className="flex-1 flex flex-col items-center">
										...
										<div className="w-full bg-gray-400 rounded-t-lg shadow-lg border-2 border-gray-500 border-b-0 pt-2 pb-3 min-h-[80px] md:min-h-[90px]">
											...
											{currentUserId === podiumOrder[0].player.user?.id && (
												<Badge
													variant="secondary"
													className="text-xs mx-auto mt-1 block w-fit"
												>
													Tú
												</Badge>
											)}
								...
								{podiumOrder[1] && (
									<div className="flex-1 flex flex-col items-center">
										...
										<div className="w-full bg-linear-to-b from-yellow-500 to-yellow-600 rounded-t-lg shadow-xl border-2 border-yellow-700 border-b-0 pt-4 pb-4 relative min-h-[120px] md:min-h-[140px] flex flex-col items-center">
											<span className="text-3xl mb-2" aria-hidden>
												🥇
											</span>
											<p className="text-sm md:text-base font-bold text-center text-yellow-900 truncate px-1">
											...
											{currentUserId === podiumOrder[1].player.user?.id && (
												<Badge
													variant="default"
													className="text-xs mx-auto mt-1 block w-fit bg-yellow-700 text-white"
												>
													Tú
												</Badge>
											)}
				...
				{rest.length > 0 && (
					<div className="space-y-2 pt-2 border-t">
						{rest.map((playerStat, index) => {
							const isCurrentUser =
								currentUserId === playerStat.player.user?.id;
							...
							return (
								...
								>
									<div className="text-lg font-bold w-10 text-center text-muted-foreground">
										{position}.
									</div>
									<Avatar className="size-10">
										<AvatarImage
											src={playerStat.player.user?.image || undefined}
