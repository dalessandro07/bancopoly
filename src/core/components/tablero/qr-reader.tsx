...
export default function QRReader() {
	...
	const handleQRCodeScanned = useCallback(
		(decodedText: string) => {
			...
			toast.error(
				"El código QR no es válido. Debe contener una URL de tablero.",
			);
			setIsOpen(false);
		},
		[router, stopScanning],
	...
	const startScanning = useCallback(async () => {
		...
		if (!element) {
			...
			toast.error(
				"Error al inicializar el escáner. Por favor, intenta de nuevo.",
			);
			setIsOpen(false);
			return;
		}
		...
		<Dialog open={isOpen} onOpenChange={handleOpenChange}>
			...
			<DialogContent className="sm:max-w-md">
				<DialogHeader>
					...
					<DialogDescription>
						Apunta la cámara hacia el código QR del tablero para acceder
						rápidamente
					</DialogDescription>
				</DialogHeader>
