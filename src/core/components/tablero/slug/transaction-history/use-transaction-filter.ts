...
export function useTransactionFilter(
	...
	const filteredTransactions = useMemo(() => {
		const isCurrentPlayerMatch = (playerId: string | null) =>
		...
		return transactions.filter((transaction) => {
			...
			const isFromBank =
				transaction.fromPlayer?.isSystemPlayer &&
				transaction.fromPlayer?.systemPlayerType === "bank";
			const isToBank =
				transaction.toPlayer?.isSystemPlayer &&
				transaction.toPlayer?.systemPlayerType === "bank";
			const isFromFreeParking =
				transaction.fromPlayer?.isSystemPlayer &&
				transaction.fromPlayer?.systemPlayerType === "free_parking";
			const isToFreeParking =
				transaction.toPlayer?.isSystemPlayer &&
				transaction.toPlayer?.systemPlayerType === "free_parking";

			if (filter === "sent") return isSender;
			if (filter === "received") return isReceiver;
			if (filter === "bank")
				return (isFromBank || isToBank) && !isSender && !isReceiver;
			if (filter === "free_parking")
				return (
					(isFromFreeParking || isToFreeParking) && !isSender && !isReceiver
				);
