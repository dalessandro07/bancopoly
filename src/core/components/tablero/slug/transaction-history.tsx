              ...
              >
                  ...
                  <div className="flex items-center gap-3 mb-3">
                    ...
                    <div className="flex items-center gap-2 flex-1 min-w-0">
                      <Avatar className={`size-8 border-2 ${isSender ? 'border-destructive' : 'border-muted'
                        }`}>
                        {fromPlayer?.user?.image ? (
                          <AvatarImage src={fromPlayer.user.image} alt={fromNameFull} />
                        ) : null}
                        <AvatarFallback className={`text-xs ${isSender ? 'bg-destructive/10 text-destructive' : 'bg-muted'
                          }`}>
                          {fromNameFull.charAt(0).toUpperCase()}
                        </AvatarFallback>
                      </Avatar>
                      <div className="flex-1 min-w-0">
                        <p className={`text-sm font-medium truncate ${isSender ? 'text-destructive' : 'text-muted-foreground'
                          }`}>
                    ...
                    {/* Flecha */}
                    <ArrowRightIcon className={`size-5 shrink-0 ${isSender ? 'text-destructive' : isReceiver ? 'text-green-500' : 'text-muted-foreground'
                      }`} />

                    {/* Jugador que recibe */}
                    <div className="flex items-center gap-2 flex-1 min-w-0">
                      <div className="flex-1 min-w-0 text-right">
                        <p className={`text-sm font-medium truncate ${isReceiver ? 'text-green-500' : 'text-muted-foreground'
                          }`}>
                          <span className="md:hidden">{toName}</span>
                          <span className="hidden md:inline">{toNameFull}</span>
                        </p>
                      </div>
                      <Avatar className={`size-8 border-2 ${isReceiver ? 'border-green-500' : 'border-muted'
                        }`}>
                        {toPlayer?.user?.image ? (
                          <AvatarImage src={toPlayer.user.image} alt={toNameFull} />
                        ) : null}
                        <AvatarFallback className={`text-xs ${isReceiver ? 'bg-green-500/10 text-green-500' : 'bg-muted'
                          }`}>
