...
export async function actionGetTableroById(slug: string) {
	try {
		...
		const tableroData = await db
			.select()
			.from(tablero)
			.where(eq(tablero.id, slug));
		...
		const creator = await db
			.select()
			.from(user)
			.where(eq(user.id, tableroData[0].userId));
...
export async function actionGetAllTableroTransactions(tableroId: string) {
	...
	try {
		...
		const playersData = await db
			...
			.from(player)
			.leftJoin(user, eq(player.userId, user.id))
			.where(or(...Array.from(playerIds).map((id) => eq(player.id, id))));

		const playersMap = new Map(
			playersData.map(({ player, user }) => [
				player.id,
				{
					...player,
					user: user || null,
 ...
 */
export async function actionGetPlayerTransactions(
	tableroId: string,
	playerId: string,
) {
	//* 1. Obtener la sesión del usuario
	...
	try {
		...
		const systemPlayers = await db
			...
			.from(player)
			.where(
				and(eq(player.tableroId, tableroId), eq(player.isSystemPlayer, 1)),
			);
		...
		const transactions = await db
			...
			.from(transaction)
			.where(
				and(eq(transaction.tableroId, tableroId), or(...transactionConditions)),
			)
			.orderBy(desc(transaction.createdAt));
...
export async function actionGetTableroStats(tableroId: string) {
	...
	try {
		...
		const tableroInitialBalance = tableroData[0].initialBalance ?? 1500;
		const playerStats = allPlayers.map(({ player, user }) => {
			const initialBalance = tableroInitialBalance;
			...
			const sentTransactions = allTransactions.filter(
				(t) => t.fromPlayerId === player.id,
			);
			const receivedTransactions = allTransactions.filter(
				(t) => t.toPlayerId === player.id,
			);
		...
		const ranking = [...playerStats].sort(
		...
		//* 10. Encontrar la mayor transferencia (excluir banco/parada libre como enviador, sí incluirlos como recibidor)
		...
		const isSystemPlayerById = new Map(
			allPlayersIncludingSystem.map(({ player }) => [
				player.id,
				player.isSystemPlayer === 1,
			]),
		);

		const eligibleTransfers = allTransactions.filter((t) => {
			if (!t.fromPlayerId || !t.toPlayerId || t.amount <= 0) return false;
			const fromIsSystem = isSystemPlayerById.get(t.fromPlayerId) ?? true;
			return !fromIsSystem;
		});
		...
		if (eligibleTransfers.length > 0) {
			const maxTransfer = eligibleTransfers.reduce(
				(max, t) => (t.amount > max.amount ? t : max),
				eligibleTransfers[0],
			);

			const fromPlayerData = allPlayersIncludingSystem.find(
				({ player }) => player.id === maxTransfer.fromPlayerId,
			);
			const toPlayerData = allPlayersIncludingSystem.find(
				({ player }) => player.id === maxTransfer.toPlayerId,
			);

			if (fromPlayerData && toPlayerData) {
				largestTransfer = {
					amount: maxTransfer.amount,
					createdAt: maxTransfer.createdAt,
		...
		//* 11. Encontrar la mayor transferencia desde Parada Libre a un jugador
		const freeParkingPlayer = allPlayersIncludingSystem.find(
			(p) =>
				p.player.isSystemPlayer === 1 &&
				p.player.systemPlayerType === "free_parking",
		);
		let largestFreeParkingTransfer = null;

		if (freeParkingPlayer) {
			const freeParkingTransfers = allTransactions.filter(
				(t) =>
					t.fromPlayerId === freeParkingPlayer.player.id &&
					t.toPlayerId &&
					t.amount > 0,
			);

			if (freeParkingTransfers.length > 0) {
				const maxFpTransfer = freeParkingTransfers.reduce(
					(max, t) => (t.amount > max.amount ? t : max),
					freeParkingTransfers[0],
				);

				const toPlayerData = allPlayersIncludingSystem.find(
					({ player }) => player.id === maxFpTransfer.toPlayerId,
				);
				if (toPlayerData && !toPlayerData.player.isSystemPlayer) {
					largestFreeParkingTransfer = {
						amount: maxFpTransfer.amount,
						createdAt: maxFpTransfer.createdAt,
						description: maxFpTransfer.description || null,
						toPlayer: {
							id: toPlayerData.player.id,
							name: toPlayerData.player.name,
							user: toPlayerData.user || null,
