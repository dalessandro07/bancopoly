...
export async function actionDeleteTablero(slug: string) {
	...
	const tableroData = await db
		.select()
		.from(tablero)
		.where(and(eq(tablero.id, slug), eq(tablero.userId, session.user.id)));
...
export async function actionDeletePlayer(playerId: string, tableroId: string) {
	...
	const tableroData = await db
		.select()
		.from(tablero)
		.where(and(eq(tablero.id, tableroId), eq(tablero.userId, session.user.id)));

	if (!tableroData[0]) {
		return {
			success: false,
			error: "No tienes permisos para eliminar jugadores de este tablero",
		};
	}
	...
	const playerData = await db
		.select()
		.from(player)
		.where(and(eq(player.id, playerId), eq(player.tableroId, tableroId)));
	...
	if (playerData[0].isSystemPlayer === 1) {
		return {
			success: false,
			error: "No se pueden eliminar jugadores del sistema",
		};
	}
...
export async function actionLeaveTablero(tableroId: string) {
	...
	if (tableroData[0].userId === session.user.id) {
		return {
			success: false,
			error: "El creador del tablero no puede salir de la sala",
		};
	}
