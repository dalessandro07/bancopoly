...
export async function actionCreateTransaction(
	...
	if (amountRaw > MAX_AMOUNT) {
		return {
			success: false,
			error: `El monto no puede superar $${MAX_AMOUNT.toLocaleString()}`,
		};
	}
	...
	if (
		fromPlayer[0].tableroId !== tableroId ||
		toPlayer[0].tableroId !== tableroId
	) {
		return {
			success: false,
			error: "Los jugadores deben pertenecer al mismo tablero",
		};
	}
	...
	if (!isFromSystemPlayer && !isFromOwnPlayer) {
		return {
			success: false,
			error: "No tienes permiso para transferir desde ese jugador",
		};
	}
	if (isFromSystemPlayer && !isCreator) {
		return {
			success: false,
			error: "Solo el creador puede usar el Banco o Parada Libre como origen",
		};
	}
