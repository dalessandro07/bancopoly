 ...
 */
export async function actionCreateTablero(
	...
	if (session.user.isAnonymous === true) {
		return {
			success: false,
			error:
				"Los usuarios invitados no pueden crear tableros. Por favor, inicia sesión con una cuenta para crear tableros.",
		};
	}

	//* 2. Obtener el nombre del tablero y dinero inicial
	const name = formData.get("name") as string;
	const initialBalanceRaw = formData.get("initialBalance") as string;
	const initialBalance = initialBalanceRaw
		? Math.max(100, parseInt(initialBalanceRaw, 10) || 1500)
		: 1500;
 ...
 */
export async function actionJoinTablero(
	initialState: unknown,
	formData: FormData,
) {
	//* 1. Obtener la sesión del usuario
	...
	if (tableroData[0].isEnded === 1) {
		return {
			success: false,
			error: "El tablero está cerrado, no puedes unirte",
		};
	}
	if (existingPlayer[0]) {
	...
	try {
		const initialBalance = tableroData[0].initialBalance ?? 1500;
