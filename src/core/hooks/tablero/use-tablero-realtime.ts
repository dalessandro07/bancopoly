...
export function useTableroRealtime({
	...
	const handlePlayerInserted = useCallback(
		(data: {
			id: string;
			tableroId: string;
			userId: string | null;
	...
	const handleTransactionInserted = useCallback(
		(data: {
			id: string;
			tableroId: string;
			fromPlayerId: string | null;
			...
			const fromPlayer = currentPlayers.find(
				(p) => p.id === newTransaction.fromPlayerId,
			);
			const toPlayer = currentPlayers.find(
				(p) => p.id === newTransaction.toPlayerId,
			);
			...
			const isToFreeParking =
				toPlayer?.isSystemPlayer &&
			...
			if (isReceiver) {
				...
				toast.success(
					`Recibiste $${newTransaction.amount.toLocaleString()} de ${fromName}`,
					{
						position: "top-center",
						duration: 2000,
					},
				...
				toast.info(
					`Enviaste $${newTransaction.amount.toLocaleString()} a ${toName}`,
					{
						position: "top-center",
					},
				);
				...
				toast.info(
					`${fromName} ${action} $${newTransaction.amount.toLocaleString()} a ${toName}`,
					{
						position: "top-center",
					},
				);
	...
	const handleTableroUpdated = useCallback(
		(data: { id: string; isEnded: boolean }) => {
			if (data.id !== tableroId) return;

			// Si el tablero fue cerrado
			if (data.isEnded) {
				toast.success(
					"El tablero ha sido cerrado. Redirigiendo a resultados...",
					{
						position: "top-center",
					},
				);
		...
		onData({ event, data, channel }) {
			...
			switch (event) {
				...
				case "tablero.tablero.updated":
					handleTableroUpdated(
						data as unknown as { id: string; isEnded: boolean },
					);
					break;
