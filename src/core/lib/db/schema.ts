...
export const tablero = sqliteTable(
	"tablero",
	{
		id: text("id").primaryKey(),
		name: text("name").notNull(),
		...
		initialBalance: integer("initial_balance").default(1500).notNull(), // Dinero inicial por jugador
...
export const transaction = sqliteTable(
	"transaction",
	{
		...
		tableroId: text("tablero_id")
			.notNull()
			.references(() => tablero.id, { onDelete: "cascade" }),
		fromPlayerId: text("from_player_id").references(() => player.id, {
			onDelete: "set null",
		}),
		toPlayerId: text("to_player_id").references(() => player.id, {
			onDelete: "set null",
		}),
